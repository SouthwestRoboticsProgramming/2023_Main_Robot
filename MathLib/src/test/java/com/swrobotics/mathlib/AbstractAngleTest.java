...
public abstract class AbstractAngleTest<T extends AbstractAngle<T>> {
    ...
    interface WrapMinMaxFn { void testWrap(double in, double min, double max, double expect); }
    ...
    interface WrapRangeFn { void testWrap(double in, double range, double expect); }
    ...
    public void test_wrapRad_minMax() {
        wrapTest((val, min, max, expect) -> assertEquals(
                create(val).wrapRad(min, max).rad(),
                expect, 0.0001
        ));
    }
    ...
    public void test_wrapRot_minMax() {
        wrapTest((val, min, max, expect) -> assertEquals(
                create(val * Math.PI * 2).wrapRot(min, max).rot(),
                expect, 0.0001
        ));
    }
    ...
    public void test_wrapRad_range() {
        wrapTest((val, range, expect) -> assertEquals(
                create(val).wrapRad(range).rad(),
                expect, 0.0001
        ));
    }
    ...
    public void test_wrapDeg_range() {
        wrapTest((val, range, expect) -> assertEquals(
                create(Math.toRadians(val)).wrapDeg(range).deg(),
                expect, 0.0001
        ));
    }
    ...
    public void test_wrapRot_range() {
        wrapTest((val, range, expect) -> assertEquals(
                create(val * Math.PI * 2).wrapRot(range).rot(),
                expect, 0.0001
        ));
    }
