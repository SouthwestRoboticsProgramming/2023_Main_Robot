...
public abstract class AbstractAngleTest<T extends AbstractAngle<T>> {
  ...
  public void test_wrapDeg_minMax() {
    wrapTest(
        (val, min, max, expect) ->
            assertEquals(create(Math.toRadians(val)).wrapDeg(min, max).deg(), expect, 0.0001));
  }
  ...
  public void test_wrapRot_minMax() {
    wrapTest(
        (val, min, max, expect) ->
            assertEquals(create(val * Math.PI * 2).wrapRot(min, max).rot(), expect, 0.0001));
  }

  @Test
  public void test_wrapRad_range() {
    wrapTest(
        (val, range, expect) -> assertEquals(create(val).wrapRad(range).rad(), expect, 0.0001));
  }
  ...
  public void test_wrapDeg_range() {
    wrapTest(
        (val, range, expect) ->
            assertEquals(create(Math.toRadians(val)).wrapDeg(range).deg(), expect, 0.0001));
  }
  ...
  public void test_wrapRot_range() {
    wrapTest(
        (val, range, expect) ->
            assertEquals(create(val * Math.PI * 2).wrapRot(range).rot(), expect, 0.0001));
  }
