...
public final class MessengerClient {
              ...
              while (!Thread.interrupted()) {
                if (prevServerHeartbeatTimestamp != -1 && connected.get()) {
                  if (System.currentTimeMillis() - prevServerHeartbeatTimestamp > TIMEOUT) {
                    System.err.println(
                        "Messenger watchdog: Force-closing socket due to" + " server timeout");
                    if (!socket.isClosed()) {
                      disconnectSocket();
  ...
  /**
   * Disconnects from the current server. After this method is called, this object should no longer
   * be used. If you want to change servers, use {@link #reconnect}.
   */
  ...
  /**
   * Prepares to send a message. This returns a {@link MessageBuilder}, which allows you to add data
   * to the message.
   *
