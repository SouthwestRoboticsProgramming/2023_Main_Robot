...
public abstract class Drivetrain extends SwitchableSubsystemBase {
  ...
  /**
   * Creates a new instance of the drivetrain. The gyroscope passed into this constructor should be
   * fully owned by this subsystem, and should not be used outside of it. To get the current
  ...
  /**
   * Adds control input to the drive base. All calls to this method each periodic are added together
   * to get the final speeds. If this is not a holonomic drive base, the horizontal movement of the
   * chassis speeds should be zero.
   *
  ...
  /**
   * Resets the current odometry pose. This method should not be used while a PathPlanner command is
   * running.
   *
  ...
  /**
   * Resets the current odometry pose regardless of whether PathPlanner is running. If calling this
   * function during PathPlanner, make sure the pose is in PathPlanner coordinates.
   *
  ...
  public void setAutoEvents(Map<String, Command> eventMap) {
    if (autoBuilder != null) throw new IllegalStateException("Auto builder already initialized");
