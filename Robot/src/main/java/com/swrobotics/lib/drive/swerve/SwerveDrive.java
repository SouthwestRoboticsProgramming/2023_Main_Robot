...
public class SwerveDrive extends Drivetrain {
  ...
  protected void drive(ChassisSpeeds speeds) {
    ...
    gyro.setSimAngle(
        gyro.getSimAngle().ccw().add(CCWAngle.rad(estimatedChassis.omegaRadiansPerSecond * 0.02)));
  ...
  protected BaseAutoBuilder createAutoBuilder(Map<String, Command> eventMap) {
    ...
    return new SwerveAutoBuilder(
        ...
        SwerveDrive.this
            ::resetPoseInternal, // Pose2d consumer, used to reset odometry at the beginning of auto
        kinematics, // SwerveDriveKinematics
        new PIDConstants(
            0.0, 0.0, 0.0), // PID constants to correct for translation error (used to create the X
        // and Y PID controllers)
        new PIDConstants(
            2.0, 0.0, 0.0), // PID constants to correct for rotation error (used to create the
        // rotation controller)
        SwerveDrive.this
            ::cheatChassisSpeeds, // Module states consumer used to output to the drive subsystem
        eventMap,
