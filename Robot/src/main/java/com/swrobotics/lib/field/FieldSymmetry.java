...
public enum FieldSymmetry {
   ...
   * line of the field. With lateral symmetry, the two sides of the field are flipped relative from
   * the driver station's point of view. An example of this is Charged Up from 2023.
   */
  LATERAL {
    @Override
    protected Pose2d flipRed(Pose2d bluePose, FieldInfo field) {
      return new Pose2d(
          ...
          new Rotation2d(
              MathUtil.wrap(Math.PI - bluePose.getRotation().getRadians(), 0, 2 * Math.PI)));
    }
  },

  /**
   * Rotational alliance symmetry. This represents symmetry as if the field was rotated 180 degrees
   * around its center. With rotational symmetry, both sides of the field are equivalent from the
   * driver station's point of view. Some examples of this are Rapid React from 2022 and Infinite
   * Recharge from 2020.
   */
  ROTATIONAL {
    @Override
    protected Pose2d flipRed(Pose2d bluePose, FieldInfo field) {
      return new Pose2d(
          ...
          new Rotation2d(
              MathUtil.wrap(bluePose.getRotation().getRadians() + Math.PI, 0, 2 * Math.PI)));
    }
  };
