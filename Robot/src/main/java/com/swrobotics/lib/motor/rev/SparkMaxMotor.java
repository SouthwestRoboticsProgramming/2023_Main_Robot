...
public abstract class SparkMaxMotor implements FeedbackMotor {
    ...
    primaryEncoder =
        new Encoder() {
          ...
          public Angle getVelocity() {
            return CWAngle.rad(
                Units.rotationsPerMinuteToRadiansPerSecond(primaryEncoderRev.getVelocity()));
          }
  ...
  public SparkMaxMotor withAlternateEncoder(
    ...
    if (alternateEncoder != null) throw new IllegalStateException("Primary encoder already set");
    ...
    if (feedbackEncoderPort == EncoderPort.ALTERNATE) pid.setFeedbackDevice(alternateEncoderRev);
    ...
    alternateEncoder =
        new Encoder() {
          ...
          public Angle getVelocity() {
            return CWAngle.rad(
                Units.rotationsPerMinuteToRadiansPerSecond(alternateEncoderRev.getVelocity()));
          }
  ...
  /**
   * Enables feedback using an absolute encoder connected to the data port. The absolute encoder can
   * not be used at the same time as the alternate encoder.
   *
   * @param type encoder type connected
   * @return this
   */
  public SparkMaxMotor withAbsoluteEncoder(SparkMaxAbsoluteEncoder.Type type) {
    if (absoluteEncoder != null) throw new IllegalStateException("Absolute encoder already set");
    ...
    absoluteEncoder =
        new Encoder() {
          ...
          public Angle getVelocity() {
            return CWAngle.rad(
                Units.rotationsPerMinuteToRadiansPerSecond(absoluteEncoderRev.getVelocity()));
          }
