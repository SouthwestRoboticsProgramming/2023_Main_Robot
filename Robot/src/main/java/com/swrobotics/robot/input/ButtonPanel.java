...
public final class ButtonPanel extends SubsystemBase {
    ...
    private final boolean[] incomingButtonStates, buttonStates, prevButtonStates;
    private final boolean[] lightStates;

    public ButtonPanel(MessengerClient msg) {
        this.msg = msg;

        incomingButtonStates = new boolean[WIDTH * HEIGHT];
        buttonStates = new boolean[WIDTH * HEIGHT];
        prevButtonStates = new boolean[WIDTH * HEIGHT];
    ...
    public boolean isButtonRising(int x, int y) {
        return buttonStates[x + y * WIDTH] && !prevButtonStates[x + y * WIDTH];
    }
    ...
    private void onButtonData(String type, MessageReader reader) {
        ...
        for (int i = 0; i < packed.length; i++) {
            for (int j = 0; j < (i == 4 ? 4 : 8); j++) {
                int x = i * 2 + j / 4;
                int y = j % 4;
                incomingButtonStates[x + y * WIDTH] = (packed[i] & (1 << j)) != 0;
            }
        }
    }
    ...
    public void periodic() {
        ...
        // Defer incoming button states until now so rising edges last one periodic
        System.arraycopy(incomingButtonStates, 0, buttonStates, 0, buttonStates.length);
