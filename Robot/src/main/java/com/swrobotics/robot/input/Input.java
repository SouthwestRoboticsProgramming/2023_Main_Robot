...
public final class Input extends SubsystemBase {
  ...
  private void driverPeriodic() {
    ...
    if (false) { // FIXME
      ...
      switch (snap.turnMode) {
        ...
        case GAME_PIECE_AIM:
          robot.limelight.setPipeline(
              getGamePiece() == GamePiece.CONE ? Limelight.CONE_PIPELINE : Limelight.CUBE_PIPELINE);
          snapAngle =
              CCWAngle.rad(
                  currentPose.getRotation().getRadians()
                      - robot.limelight.getXAngle().getRadians());
      ...
      if (snap.turnMode == SnapPositions.TurnMode.GAME_PIECE_AIM) shouldBeRobotRelative = true;
    } else {
      setCommandEnabled(snapTurnCmd, false);
      shouldBeRobotRelative = false;
    }
