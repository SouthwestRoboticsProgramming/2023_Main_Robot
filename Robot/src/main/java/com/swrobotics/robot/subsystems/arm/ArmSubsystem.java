...
public final class ArmSubsystem extends SwitchableSubsystemBase {
  ...
  public ArmSubsystem(MessengerClient msg) {
    if (RobotBase.isSimulation()) {
      ...
      bottomJoint =
          new PhysicalJoint(
              BOTTOM_MOTOR_ID, BOTTOM_CANCODER_ID, BOTTOM_GEAR_RATIO, armInfo.bottomOffset, true);
      topJoint =
          new PhysicalJoint(
              TOP_MOTOR_ID, TOP_CANCODER_ID, TOP_GEAR_RATIO, armInfo.topOffset, false);
    }
    ...
    currentVisualizer =
        new ArmVisualizer(
            extent / 2, extent / 2, mechanism, "Current Arm", Color.kDarkGreen, Color.kGreen);
    targetVisualizer =
        new ArmVisualizer(
            extent / 2, extent / 2, mechanism, "Target Arm", Color.kDarkRed, Color.kRed);
