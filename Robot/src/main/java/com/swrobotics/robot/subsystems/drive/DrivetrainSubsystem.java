  ...
  public static final class StateVisualizer {
    ...
    public void update() {
      ...
      for (int i = 0; i < ligaments.length; i++) {
        ...
        ligament.setLength(
            state.speedMetersPerSecond / 4.11 + Math.copySign(0.05, state.speedMetersPerSecond));
        ...
        if (state.speedMetersPerSecond == 0) {
          ...
          outputAngle =
              new Translation2d(state.speedMetersPerSecond, 0).rotateBy(state.angle).getAngle();
        }
