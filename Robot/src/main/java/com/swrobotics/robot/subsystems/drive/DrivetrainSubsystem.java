...
public class DrivetrainSubsystem extends SwitchableSubsystemBase implements StatusLoggable {
  ...
  public DrivetrainSubsystem() {
    ...
    modules =
        new SwerveModule[] {
          ...
          new SwerveModule(
              FRONT_LEFT_SELECT.getSelected(), new Translation2d(0.3, 0.3), 0.0), // Front left
          new SwerveModule(
              FRONT_RIGHT_SELECT.getSelected(), new Translation2d(0.3, -0.3), 90.0), // Front right
          new SwerveModule(
              BACK_LEFT_SELECT.getSelected(), new Translation2d(-0.3, 0.3), 180.0), // Back left
          new SwerveModule(
              BACK_RIGHT_SELECT.getSelected(), new Translation2d(-0.3, -0.3), 270.0) // Back right
        };
  ...
  public boolean isMoving() {
    ...
    Translation2d translation =
        new Translation2d(currentMovement.vxMetersPerSecond, currentMovement.vyMetersPerSecond);
  ...
  public SwerveAutoBuilder getAutoBuilder(HashMap<String, Command> eventMap) {
    ...
    SwerveAutoBuilder autoBuilder =
        new SwerveAutoBuilder(
            ...
            new PIDConstants(
                0.0, 0.0, 0.0), // PID constants to correct for translation error (used to
            // create the X
            // and Y PID controllers)
            new PIDConstants(
                2.0, 0.0, 0.0), // PID constants to correct for rotation error (used to create
            // the
            // rotation controller)
