...
public class ImGuiImplGlfw {
  ...
  private final class CreateWindowFunction extends ImPlatformFuncViewport {
    ...
    public void accept(final ImGuiViewport vp) {
      ...
      glfwWindowHint(
          GLFW_DECORATED, vp.hasFlags(ImGuiViewportFlags.NoDecoration) ? GLFW_FALSE : GLFW_TRUE);
      if (glfwHawWindowTopmost) {
        glfwWindowHint(
            GLFW_FLOATING, vp.hasFlags(ImGuiViewportFlags.TopMost) ? GLFW_TRUE : GLFW_FALSE);
      }

      data.window =
          glfwCreateWindow(
              (int) vp.getSizeX(), (int) vp.getSizeY(), "No Title Yet", NULL, windowPtr);
