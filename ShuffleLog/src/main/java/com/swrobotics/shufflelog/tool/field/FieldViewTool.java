...
public final class FieldViewTool extends ViewportTool {
  ...
  public FieldViewTool(ShuffleLog log) {
    ...
    super(
        log, "Field View", ImGuiWindowFlags.NoScrollbar | ImGuiWindowFlags.NoScrollWithMouse, P3D);
  ...
  protected void drawViewportContent(PGraphics pGraphics) {
    ...
    if (viewMode.get() == MODE_3D) {
      projection.set(
          new Matrix4f()
              .perspective((float) Math.toRadians(80), g.width / (float) g.height, 0.01f, 1000f));
    } else {
      float normalScale =
          Math.min(calcReqScale(g.width, (float) WIDTH), calcReqScale(g.height, (float) HEIGHT));
      float rotatedScale =
          Math.min(calcReqScale(g.width, (float) HEIGHT), calcReqScale(g.height, (float) WIDTH));
  ...
  protected void drawGuiContent() {
    if (ImGui.beginTable("layout", 2, ImGuiTableFlags.BordersInner | ImGuiTableFlags.Resizable)) {
      ...
      ImGui.text("Tool:");
      ImGui.sameLine();
      if (ImGui.radioButton("Move", gizmoOp == Operation.TRANSLATE)) gizmoOp = Operation.TRANSLATE;
      ImGui.sameLine();
      if (ImGui.radioButton("Rotate", gizmoOp == Operation.ROTATE)) gizmoOp = Operation.ROTATE;
      ...
      if (gizmoTarget != null) {
        ...
        ImGuizmo.manipulate(
            view.getColumnMajor(), projection.get().getColumnMajor(), transArr, gizmoOp, gizmoMode);
        if (ImGuizmo.isUsing()) {
      ...
      if (mouseX >= 0 && mouseY >= 0 && mouseX < size.x && mouseY < size.y) {
        ...
        if (viewMode.get() == MODE_2D) {
          ...
          mouseRay =
              new Ray3f(
                  new Vector3f(0, 0, 0), new Vector3f(eyeSpace.x, eyeSpace.y, -1).normalize());
        }
