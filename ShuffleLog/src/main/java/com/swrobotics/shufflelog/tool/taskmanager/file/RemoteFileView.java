...
public final class RemoteFileView {
  ...
  private void uploadFile(File file, String targetDirPath) {
    ...
    if (file.isFile()) {
      try {
        ...
        msg.prepare(name + MSG_WRITE_FILE).addString(path).addInt(data.length).addRaw(data).send();
      } catch (IOException e) {
  ...
  private void showDirectory(RemoteDirectory dir, boolean isRoot) {
    ...
    boolean open =
        ImGui.treeNodeEx(
            dirName,
            ImGuiTreeNodeFlags.SpanFullWidth | (isRoot ? ImGuiTreeNodeFlags.DefaultOpen : 0));
